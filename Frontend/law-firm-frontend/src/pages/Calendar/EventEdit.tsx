// src/pages/Calendar/EventEdit.tsx
...
export const EventEdit: React.FC = () => {
  ...
  const { selectedEvent, isLoading, fetchEventById, updateEvent, clearSelectedEvent } = useCalendarStore();
  ...
  useEffect(() => {
    if (selectedEvent) {
      reset({
        ...
        startDateTime: new Date(selectedEvent.startDateTime).toISOString().slice(0, 16),
        endDateTime: new Date(selectedEvent.endDateTime).toISOString().slice(0, 16),
        isAllDay: selectedEvent.isAllDay,
        color: selectedEvent.color || '#3b82f6',
        matterId: selectedEvent.matterId,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button
          onClick={() => navigate(`/calendar/events/${id}`)}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end gap-3">
          <Button type="button" variant="outline" onClick={() => navigate(`/calendar/events/${id}`)}>
            Cancel
          </Button>
