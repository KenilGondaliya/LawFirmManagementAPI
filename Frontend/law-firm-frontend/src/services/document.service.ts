...
export const documentService = {
  ...
  uploadDocument: async (data: UploadDocumentDto): Promise<Document> => {
    ...
    if (data.documentTypeId) formData.append('documentTypeId', data.documentTypeId.toString());
  ...
  createVersion: async (documentId: number, file: File, changeSummary?: string): Promise<DocumentVersion> => {
    const formData = new FormData();
  ...
  shareDocument: async (
    documentId: number,
    options: { userId?: number; email?: string; permission?: string; expiresInDays?: number }
    ...
    if (options.expiresInDays) params.append('expiresInDays', options.expiresInDays.toString());
    
    const response = await api.post(`/documents/${documentId}/share?${params.toString()}`);
    return response.data;
  },
  ...
  addComment: async (documentId: number, comment: string): Promise<DocumentComment> => {
  ...
  moveDocument: async (documentId: number, folderId?: number): Promise<void> => {
    const params: any = {};
    if (folderId !== undefined) params.folderId = folderId;
    await api.post(`/documents/${documentId}/move?${new URLSearchParams(params).toString()}`);
  },
